      ...
      <div className="container mx-auto px-4 max-w-7xl flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 sm:gap-5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Dark Mode Toggle - Expanding Design */}
            <button
              onClick={toggleTheme}
              className="relative flex items-center h-9 p-1 rounded-full bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-all duration-500 hover:ring-4 hover:ring-blue-500/10 group overflow-hidden w-9 hover:w-24"
            >
              <div className={`flex items-center justify-center size-7 rounded-full shadow-md transition-all duration-500 z-10 ${theme === 'dark' ? 'translate-x-full bg-gray-700 text-yellow-300 rotate-[360deg]' : 'translate-x-0 bg-white text-orange-500'}`}>
                {theme === 'light' ? (
              ...
              <span className={`absolute left-9 text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap text-gray-500 dark:text-gray-400 ${theme === 'dark' ? 'left-3' : 'left-9'}`}>
