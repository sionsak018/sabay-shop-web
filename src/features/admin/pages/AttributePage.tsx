    ...
    <div className="space-y-6 text-left">
        ...
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead className="bg-gray-50 dark:bg-gray-800/50 border-b dark:border-gray-800">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Type</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Options</th>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
            {loading ? (
              ...
              <tr key={attr.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                <td className="px-6 py-4">
                    <span className="text-sm font-bold text-gray-800 dark:text-gray-200 block">{attr.name}</span>
                </td>
                <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-tighter">{attr.type}</span>
                </td>
                <td className="px-6 py-4">
                   <div className="flex flex-wrap gap-2">
                      ...
                      {!attr.options?.length && <span className="text-xs text-gray-300 dark:text-gray-600">-</span>}
                   </div>
                </td>
                <td className="px-6 py-4 text-right space-x-2">
                  <button onClick={() => handleOpenModal(attr)} className="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/10 rounded-md transition-colors">
                  ...
                  <button onClick={() => handleDelete(attr.id)} className="p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-md transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#1c1c1d] rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden border dark:border-gray-800 animate-in zoom-in-95 duration-300">
            <div className="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 border-b dark:border-gray-800 flex justify-between items-center flex-shrink-0">
              <h2 className="text-sm font-black text-gray-800 dark:text-gray-100 uppercase tracking-widest">{editingAttr ? 'Edit Custom Field' : 'Create New Field'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="w-8 h-8 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 rounded-full transition-all">
              ...
              <form id="attr-form" onSubmit={handleSubmit} noValidate className="p-4 sm:p-8 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-gray-50 dark:bg-[#08060d] border rounded-xl text-sm font-bold outline-none transition-all dark:text-gray-200 ${errors.name ? 'border-red-300' : 'border-gray-200 dark:border-gray-800 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10'}`}
                    placeholder="Enter field name..."
                  />
                  {errors.name && <p className="text-red-500 text-[10px] font-bold mt-1.5 ml-1">{errors.name}</p>}
                </div>
              <div>
                <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase mb-2 tracking-widest">Input Type for Users</label>
                <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full px-4 py-3 bg-gray-50 dark:bg-[#08060d] border border-gray-200 dark:border-gray-800 rounded-xl text-sm font-bold outline-none cursor-pointer focus:bg-white dark:focus:bg-[#08060d] focus:border-blue-500 dark:focus:border-blue-400 transition-all dark:text-gray-200">
              ...
              {formData.type === 'select' && (
                <div className="space-y-4 pt-4 border-t border-dashed border-gray-100 dark:border-gray-800">
                  <div className="flex items-center justify-between">
                    <label className={`block text-[10px] font-black uppercase tracking-widest ${errors.options ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}>Define Options & Icons</label>
                    <button type="button" onClick={() => {
                        handleAddOption();
                        if (errors.options) setErrors(prev => ({ ...prev, options: '' }));
                    }} className="text-[10px] font-black text-blue-600 dark:text-blue-400 hover:text-blue-700 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-full uppercase tracking-tighter transition-colors">+ Add Option</button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {formData.options.map((opt, idx) => (
                        <div key={idx} className={`flex items-center gap-3 bg-gray-50/50 dark:bg-gray-800/20 p-3 rounded-2xl border transition-all ${errors.options && !opt.value.trim() ? 'border-red-300 bg-red-50/30' : 'border-gray-100 dark:border-gray-800'}`}>
                            <div className="relative group size-12 flex-shrink-0">
                                { (opt.image || opt.image_url) && (
                                    <button
                                        ...
                                        className="absolute -top-1 -right-1 z-10 size-5 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-red-600 transition-colors border-2 border-white dark:border-gray-900 scale-100 sm:scale-0 sm:group-hover:scale-100 transition-transform"
                                    >
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
                                    </button>
                                )}
                                <label className="size-12 rounded-full border-2 border-dashed border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex items-center justify-center cursor-pointer hover:border-blue-500 dark:hover:border-blue-400 transition-all overflow-hidden shadow-sm">
                                    {opt.image || opt.image_url ? (
                                        <img
                            ...
                            <input
                                ...
                                className="flex-1 min-w-0 bg-transparent text-sm font-bold outline-none border-b-2 border-transparent focus:border-blue-500 transition-all py-1 dark:text-gray-200"
                                placeholder="Option label (e.g. Toyota)"
                            />

                            <button type="button" onClick={() => handleRemoveOption(idx)} className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-500 transition-colors">
